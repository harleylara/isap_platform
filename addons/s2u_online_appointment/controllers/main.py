...
class OnlineAppointment(http.Controller):
    ...
    def appointee_id_to_partner_id(self, appointee_id):
        ...
        appointee = request.env['res.users'].sudo().search(
            [('id', '=', appointee_id)])
        if appointee:
        ...
        if form_data:
            ...
            try:
                appointment_date = datetime.datetime.strptime(
                    form_data['appointment_date'], '%d/%m/%Y').strftime('%d/%m/%Y')
            except:
                appointment_date = ''
            ...
            if appointee_id and appointment_option_id and appointment_date:
                free_slots = self.get_free_appointment_slots_for_day(
                    appointment_option_id, form_data['appointment_date'], appointee_id, criteria)
                days_with_free_slots = self.get_days_with_free_slots(appointment_option_id,
                                                                     appointee_id,
                                                                     datetime.datetime.strptime(
                                                                         form_data['appointment_date'], '%d/%m/%Y').year,
                                                                     datetime.datetime.strptime(
                                                                         form_data['appointment_date'], '%d/%m/%Y').month,
                                                                     criteria)
                values.update({
    ...
    def online_appointment(self, **kw):
        ...
        values = self.prepare_values(
            default_appointee_id=kw.get('appointee', False))
    ...
    def check_slot_is_possible(self, option_id, appointment_date, appointee_id, slot_id):
        ...
        date_start = datetime.datetime.strptime(
            appointment_date, '%d/%m/%Y').strftime('%Y-%m-%d')
        ...
        event_start = self.ld_to_utc(date_start + ' ' + functions.float_to_time(
            slot.slot), appointee_id).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def free_timeslots(self, appointment_option, appointment_with, appointment_date, form_criteria, **kwargs):
        ...
        if option_id:
            option = request.env['s2u.appointment.option'].sudo().browse(
                option_id)
        ...
        free_slots = self.get_free_appointment_slots_for_day(
            option_id, appointment_date, appointee_id, form_criteria)
    ...
    def online_appointment_portal_cancel(self, **post):
        ...
        if id:
            appointment = request.env['s2u.appointment.registration'].search([
                                                                             ('id', '=', id)])
            if appointment and (
                ...
                self.online_appointment_state_change(
                    appointment, previous_state)
