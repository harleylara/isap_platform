...
class RecruitmentSource(models.Model):
    ...
    source_id = fields.Many2one(
        'utm.source', "Source", ondelete='cascade', required=True)
    email = fields.Char(related='alias_id.display_name',
...
class RecruitmentStage(models.Model):
    ...
    def _compute_is_warning_visible(self):
        ...
        applicants = dict((data['stage_id'][0], data['stage_id_count'])
...
class RecruitmentDegree(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", default=1, help="Gives the sequence order when displaying a list of degrees.")


class ApplicantCountry(models.Model):
    _name = "hr.applicant.country"
    _description = "Applicant Country"
    _sql_constraints = [
        ('name_uniq', 'unique (name)',
         'The name of the Country of Applicant must be unique!')
    ]

    name = fields.Char("Country", required=True, translate=True)
    sequence = fields.Integer(
        "Sequence", default=1, help="Gives the sequence order when displaying a list of countris.")
...
class Applicant(models.Model):
    ...
    active = fields.Boolean(
        "Active", default=True, help="If the active field is set to false, it will allow you to hide the case without removing it.")
    ...
    company_id = fields.Many2one(
        'res.company', "Company", compute='_compute_company', store=True, readonly=False, tracking=True)
    user_id = fields.Many2one(
        'res.users', "Recruiter", compute='_compute_user',
        tracking=True, store=True, readonly=False)
    date_closed = fields.Datetime("Hire Date", compute='_compute_date_closed',
                                  store=True, index=True, readonly=False, tracking=True)
    ...
    priority = fields.Selection(
        AVAILABLE_PRIORITIES, "Appreciation", default='0')
    job_id = fields.Many2one(
        'hr.job', "Applied Job", domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]", tracking=True)
    salary_proposed_extra = fields.Char(
        "Proposed Salary Extra", help="Salary Proposed by the Organisation, extra advantages", tracking=True)
    salary_expected_extra = fields.Char(
        "Expected Salary Extra", help="Salary Expected by Applicant, extra advantages", tracking=True)
    salary_proposed = fields.Float("Proposed Salary", group_operator="avg",
    ...
    delay_close = fields.Float(compute="_compute_day", string='Delay to Close',
                               readonly=True, group_operator="avg", help="Number of days to close", store=True)
    color = fields.Integer("Color Index", default=0)
    emp_id = fields.Many2one('hr.employee', string="Employee",
                             help="Employee linked to the applicant.", copy=False)
    ...
    attachment_ids = fields.One2many('ir.attachment', 'res_id', domain=[
                                     ('res_model', '=', 'hr.applicant')], string='Attachments')
    ...
    legend_done = fields.Char(
        related='stage_id.legend_done', string='Kanban Valid')
    legend_normal = fields.Char(
        related='stage_id.legend_normal', string='Kanban Ongoing')
    application_count = fields.Integer(
        compute='_compute_application_count', help='Applications with the same email')
    ...
    type_id_country = fields.Many2one('hr.applicant.country', "Country")
    partner_birthday = fields.Date("Birthday")
    ...
    def _compute_application_count(self):
        ...
        application_data_mapped = dict(
            (data['email_from'], data['email_from_count']) for data in application_data)
    ...
    def _notify_get_reply_to(self, default=None, records=None, company=None, doc_names=None):
        ...
        aliases = self.mapped('job_id')._notify_get_reply_to(
        ...
        if leftover:
            res.update(super(Applicant, leftover)._notify_get_reply_to(
    ...
    def message_new(self, msg, custom_values=None):
        ...
        if custom_values and 'job_id' in custom_values:
            stage = self.env['hr.job'].browse(
                custom_values['job_id'])._get_first_stage()
